...
* Our hypothesis for this section is that similar photos get uploaded for similiar businesses. And we've taken the liberty of tagging photos of 30K Yelp reviews using Google's SOMETHING
* Our challenge will be to enable users to select an image they like and then show them similarly tagged images.
...
* There will be a short quiz to evalute progress.
* You should understand what weights are used to assign labels for label propgation and how to avoid or correct an endless cycle.
* You should also be familiar with why seed labels are sometimes used.
